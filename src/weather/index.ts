...
export const getSevenDayForecast = async (location: string) => {
    ...
    const response =  await makeWeatherApiCall<SevenDayForecastResponse>(forecastUrl)

    // format the response to be sent to telegram
    return formatWeeklyForecast(response)

}


function formatWeeklyForecast(forecast: SevenDayForecastResponse): string {
